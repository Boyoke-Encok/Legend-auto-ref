...
def get_random_name():
    first_names = [
    "Indah", "Dasuki", "Supri", "Endang", "Joko", "Willi", "Basuki", "Luna", "Heri", "Alex",
    "Marsha", "Maya", "Ella", "Nur", "Jamal", "Kliwon", "Henny", "Sebastian", "Aida", "Evy",
    "Agung", "Harto", "Oki", "Lukman", "Intan", "Jono", "Jamaludin", "Lukek", "Juli", "Levis",
    "Adam", "Gabriel", "Mateo", "Samuel", "David", "Anthony", "Jaxon", "Christopher", "Andrew", "Lincoln",
...
def post_to_waitlist(session, email):
    try:
        ...
        if response.status_code == 201:
            print(Fore.GREEN + f"Successfully sign up to waitlist: {first_name} {last_name}")
        else:
            print(Fore.RED + "Failing to register to waitlist:", response.json())
    except requests.RequestException:
